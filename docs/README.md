# Documentación de Joss 3.6

Esta referencia describe el comportamiento implementado en el repositorio. Las APIs de plugins no se presentan como parte del núcleo. La documentación publicada está en [joss.red/docs](https://joss.red/docs) y el registro en [joss.red/pub](https://joss.red/pub).

## Lenguaje

- [Sintaxis](SINTAXIS.md)
- [Sistema de tipos e inferencia](SISTEMA_TIPOS.md)
- [Arquitectura del lenguaje](ARQUITECTURA.md)
- [Módulos, archivos e imports](MODULOS_IMPORTS.md)
- [Concurrencia](CONCURRENCIA.md)
- [Módulos nativos y builtins](MODULOS_NATIVOS.md)
- [Estado y límites de implementación](ESTADO_IMPLEMENTACION.md)

## Desarrollo

- [CLI](CLI.md)
- [Análisis estático](ANALIZADOR.md) y [diagnósticos](DIAGNOSTICOS.md)
- [Auditoría técnica y alineación con la tesis](AUDITORIA_TECNICA_2026.md)
- [Estructura de proyectos](ESTRUCTURA_PROYECTO.md)
- [Configuración](CONFIGURACION.md)
- [Controladores y HTTP](CONTROLADORES.md)
- [Servidor](SERVIDOR.md)
- [Middleware](MIDDLEWARE.md)
- [Vistas](VISTAS.md)
- [Datos y modelos](MODELOS.md)
- [Migraciones](MIGRACIONES.md) y [Schema Builder](SCHEMA_BUILDER.md)
- [Assets](ASSETS.md), [SEO y sitemap](SEO_SITEMAP.md), [WebSockets](WEBSOCKETS.md)

## Extensibilidad

- [Plugins y JP v2](PLUGINS.md)
- [Extensión de VS Code](VSCODE_EXTENSION.md)

Los plugins oficiales se instalan aparte: [joss_ai](https://github.com/josprox/joss_ai), [joss_smtp](https://github.com/josprox/joss_smtp), [joss_notify](https://github.com/josprox/joss_notify) y [joss_backup](https://github.com/josprox/joss_backup).
