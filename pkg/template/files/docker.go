...
func GetDockerFiles(path string) map[string]string {
	return map[string]string{
		filepath.Join(path, "Dockerfile"): `FROM debian:bookworm-slim
...
`,
		filepath.Join(path, "docker-compose.yml"): `services:
  app:
    build: .
    volumes:
      - .:/app
    ...
    environment:
      - APP_ENV=development
`,
		filepath.Join(path, "docker-entrypoint.sh"): `#!/bin/bash
set -e
